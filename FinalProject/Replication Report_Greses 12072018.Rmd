...
[Publication in Rpubs](http://rpubs.com/Greses88/446795)
...
> Pre-registration - update after talking with Mike and add link here 
...
> Predictions or hipothesis of what I want to see (different from author, optional)
> I am going to restric my analysis on this, and then this is my code 
...
summarise(mean = mean(`if (...) NULL`, na.rm = T))
Esquema
```

```{r}
Esquema1 <- Esquema %>%
  spread(Scheme.Quality, mean)
Esquema1
```
...
Esquema2 <- Esquema1 %>%
  summarise(intermediate_freq = sum(Intermediate),
            intermediate_perc = round((mean(Intermediate)*100),0),
            poor_freq = sum(Poor),
            poor_perc = round((mean(Poor)*100),0))
Esquema2
```
